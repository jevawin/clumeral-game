...
describe('resolveRoute (RTE-03, ARC-03)', () => {
  ...
  it('RTE-03: /play with today already solved → solved (post-solve home)', () => {
    const todayEntry = { date: '2026-05-03', tries: 2 };
    expect(resolveRoute('/play', ctx({ hasData: true, todayEntry }))).toEqual({ kind: 'solved' });
  });

  it('RTE-03: /welcome with today already solved → solved (post-solve home)', () => {
    const todayEntry = { date: '2026-05-03', tries: 2 };
    expect(resolveRoute('/welcome', ctx({ hasData: true, todayEntry }))).toEqual({ kind: 'solved' });
  });
  ...
  it('RTE-03: /welcome (no todayEntry) → welcome (passthrough)', () => {
    expect(resolveRoute('/welcome', ctx({ todayEntry: null }))).toEqual({ kind: 'welcome' });
  });
