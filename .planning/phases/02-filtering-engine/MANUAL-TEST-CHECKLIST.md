...
```
- [x] gameRows.length === 900
- [x] gameHeaders.length === 23
...
```
- [x] gameHeaders[4] is a string (the column header name)
- [x] gameRows[0][gameHeaders[4]] is a string like "a prime number" or "a square or a cube number" (NOT a number)
...
```
- [x] 6 keys: SpecialNumbers, Sums, AbsoluteDifference, Products, Means, Range
- [x] SpecialNumbers: [4, 5, 6]
- [x] Sums: [7, 8, 9, 10]
- [x] AbsoluteDifference: [11, 12, 13]
- [x] Products: [14, 15, 16, 17]
- [x] Means: [18, 19, 20, 21]
- [x] Range: [22]
...
```
- [x] All 5 calls return (no hang)
- [x] Answers differ across runs (randomness is working)
- [x] Clue labels differ across runs
...
```
- [x] Text clue operators contain ONLY '=' and '!=' (no '<=' or '>=')
- [x] Numeric clue operators include at least some of: '<=', '=', '!=', '>='
...
```
- [x] No console.error messages appeared
- [x] All 20 answers are numbers between 100 and 999
...
```
- [x] results.length === 50 (all 50 calls terminated)
- [x] All valid numbers (100–999)
- [x] No consecutive duplicates (random)
...
```
- [x] row111 found (not undefined)
- [x] runFilterLoop([row111]) returns without hanging
- [x] pathResult.answer === 111 (only candidate is the answer)
...
```
- [x] answer is a number between 100 and 999
- [x] clues.length >= 1
- [x] Each clue has label, operator, value properties
- [x] Each label is a non-empty string (human-readable column header)

---

## Sign-off

- [x] All FILT-01 through FILT-10 checks above are ticked
- [x] 50-iteration stress test passed (no consecutive duplicates, all 100-999)
- [x] Pathological input test (row 111) passed without hanging
